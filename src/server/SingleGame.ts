...
class SingleGame extends GameAbstract {
    ...
    makeBotShot() {
        ...
        bot.shots[this.round] = new Position(Math.floor(Math.random() * 9), Math.floor(Math.random() * 9))
    }
    ...
    joinPlayer(player: Player): void {
        ...
        const bot = new Player(playerId, grid, ships)
